var TimePicker = (function () {
	...
	TimePicker.prototype.render = function () {
		...
		for (var i = 0; i < 1440; i += this.step) {
			...
			html += '<div' + (viewDate.getHours() == this.date.getHours() && viewDate.getMinutes() == this.date.getMinutes() ? ' class="picked"' : '') + '>' + to2(viewDate.getHours()) + ':' + to2(viewDate.getMinutes()) + ':' + to2(viewDate.getSeconds()) + '</div>'
	...
	var
		...
		stringToDate = function (str, format) {
			...
			var date = new Date(1900, 0, 0, Number(tokens[0]), Number(tokens[1], tokens.length > 2 ? Number(tokens[2]) : 0))
