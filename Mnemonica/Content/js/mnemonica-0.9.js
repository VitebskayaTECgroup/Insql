...
/**
 * Перечисление. Самописное, потому что старые браузеры
 * @param {ArrayLike} iterable массив или похожая структура
 * @param {(this: ArrayLike, x: any, i: number) => void} callback обработчик для каждого элемента. В него передаются объект и положение в массиве
 */
function forEach(iterable, callback) {
	for (var i = 0; i < iterable.length; i++) callback.call(iterable, iterable[i], i)
}

/**
 * Получение ключей объекта как массив. Самописное, потому что старые браузеры
...
function Mnemo(container) {
	...
	var settings = {
		...
		// scada-функции
		db: container.getAttribute('db') || 'insql',
	...
	var Storage = (function () {
		...
		function save(withReload) {
			...
			$.post(path + 'save.aspx?id=' + scheme, storage.innerHTML, function () {
	...
	if ((settings.users.length + settings.admins.length) > 0) {
		$.post(path + 'auth.aspx', {}, function (text) {
	...
	if (container.hasAttribute('lib')) {
		$.get(container.getAttribute('lib') + '.json', {}, function (json) {
			try {
				Library = json
				readyToWork[0] = true
			} catch (e) {
	...
	var Server = (function () {
		...
		function load() {
			console.log(path + settings.db + '/live.aspx', settings, path)
			$.post(path + settings.db + '/live.aspx', { tags: tags }, function (data) {
				try {
					eval(data)
	...
	if (container.innerHTML == '') {
		$.get(scheme + '.html?r=' + Math.random(), {}, function (html) {
	...
	// Контекстное меню на добавление объектов, если администратор
	function EditorContext() {
		...
		$(document).on('contextmenu', function (ev) {
			...
			if (Library.movable) {
				forEach(objectKeys(Library.movable), function (mov) {
					try {
			...
			if (settings.admin) {
				if (Library.images) {
					forEach(objectKeys(Library.images), function (imageType) {
						try {
							var defState = objectKeys(Library.images[imageType])[0]
							actions['Image: ' + imageType] = function (e) {

								// Загружаем список всех тегов
								$.post(path + 'local/tags.aspx').done(function (json) {
									var select, button
									Dialog(e, h('div',
										h('div',
											select = h('select',
												...
												h('option', {
													value: '',
													innerHTML: '<i>создать новый тег</i>'
												}),
												$.map(json, function (tag) {
													return h('option', {
														value: tag.TagName,
														innerHTML: tag.TagName + ": " + tag.Description
													})
												})
											),
											button = h('button',
				...
				if (Library.controls) {
					forEach(objectKeys(Library.controls), function (ctrl) {
						try {
							var defState = objectKeys(Library.controls[ctrl])[0]
			...
			ContextMenu(ev.originalEvent, actions)
		})
	}

	// Проверка на спец-классы, при наличии которых включается мограние значений
	function Blink() {
		forEach(container.querySelectorAll('[blinkcount]'), function (el) {
			var count = Number(el.getAttribute('blinkcount'))
			if (count < settings.blinkCount) {
				var cls = el.className.split(' ')
				for (var j = 0; j < cls.length; j++) {
					if (cls[j].toLowerCase().indexOf('blink_') > -1) {
						if (cls[j].indexOf('_inverted') > -1) {
							el.className = el.className.replace('_inverted', '')
						} else {
							el.className = el.className.replace(cls[j], cls[j] + '_inverted')
						}
						break;
					}
				}
				count++
				el.setAttribute('blinkcount', '' + count)
			} else {
				el.className = el.className.replace('_inverted', '')
				el.removeAttribute('blinkcount')
			}
		})
	}

	// Непродолжительная подсветка для новых значений
	function Highlight() {
		forEach(container.querySelectorAll('[highlight]'), function (el) {
			var count = Number(el.getAttribute('highlight'))
			if (count <= 0) {
				el.removeAttribute('highlight')
			}
			else {
				el.setAttribute('highlight', String(--count))
			}
		})
	}
	...
	function Log() {
		$.post(settings.db.history, { tag: Server.all(), take: 20, skip: 0 }, function (json) {
	...
	/**
	 * Блок, отображающий значение, является базовым для INSQL схем
	 * Обычно строка, но может быть изображением или шкалой с динамическим сдвигом
	 * @param {HTMLElement} el HTML элемент, отображаемый на схеме */
	function Item(el) {
		...
		Server.add(this.tags)
		Server.add(this.checks)
		...
		$(this.el).on('contextmenu', function (e) {
			...
			forEach(self.tags, function (tag) {
				actions[tag] = function () {
					var a = h('a', {
						target: '_blank',
						href: '/asu/pages/query/#t=' + tag + '|q=' + tag + '|mode=table',
						style: { display: 'none' }
					})
			...
			ContextMenu(e.originalEvent, actions)
		})
	...
	/** 
	 * Статичный объект, который можно размещать через перетаскивание
	 * @param {HTMLElement} el HTML элемент, отображаемый на схеме */
	function Static(el) {
	...
	function ImageElement(el, x, y, imageType, imageTag, defState) {
		...
		if (!el) {
			...
			$(el).on('contextmenu', function (e) {
				...
				if (settings.editor) {
					forEach(objectKeys(self.states), function (key) {
						actions[self.states[key].text] = function () {
							$.post(path + 'local/write.aspx', {
								tagName: self.imageTag,
								value: key
							}, function (json) {
								try {
									Server.load()
			...
			el.onclick = function () {
				...
				$.post(path + 'local/history.aspx', { tags: self.imageTag, take: 10, skip: 0 })
					.done(function (json) {
...
// Перезагрузка, чтобы избежать утечек памяти, если схема открыта очень долго
