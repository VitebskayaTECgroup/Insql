{минутные срезы}
/*ReportName="ОСВ через конденсатор ТГ3" (минутные срезы)*/
/*Mode=минутные*/
...
SELECT
	DateTime as "Дата",
	null,
	CAST(U03_UM21F001*U03_SY00A700 as decimal(15,2)) as "G ОСВ ТГ3, т/ч [U03_UM21F001]"
FROM
	OpenQuery(INSQL,
	"SELECT DateTime,  U03_SY00A700, U03_UM21F001
...
{часовые срезы}
/*ReportName="ОСВ через конденсатор ТГ3" (срезы на конец часа)*/
/*Mode=часовые-1*/
...
SELECT
	...
	null,
	CAST(U03_UM21F001_H1 as decimal(15,2)) as "G ОСВ ТГ3 за п/ч, т/ч [U03_UM21F001_H1]"
FROM
	OpenQuery(INSQL,
...
{суточные срезы}
/*ReportName="ОСВ через конденсатор ТГ3" (срезы на конец суток)*/
/*Mode=суточные-1*/
...
SELECT
	...
	null,
	CAST(U03_UM21F001_S1 as decimal(15,2)) as "G ОСВ ТГ3 за п/c, т/ч [U03_UM21F001_S1]"
FROM
	OpenQuery(INSQL,
...
/*ReportName="ОСВ через конденсатор ТГ3" (часовые срезы по сменам 8/12)*/
/*Mode=8-12*/
/*DownQuery="минутные срезы"*/
/*UpQuery="посменный"*/
...
{посменный}
/*ReportName="ОСВ через конденсатор ТГ3"(агрегированные часовые данные по сменам)*/
/*Mode=посменный*/
...
SELECT
	...
	CAST(sum(U03_UM21F001_H1) as decimal(15,2)) as "G ОСВ ТГ3 за п/ч, т/см [U03_UM21F001_H1]"
FROM
	OpenQuery(INSQL,
	"SELECT DateTime,
...
{смены за месяц}
/*ReportName="ОСВ через конденсатор ТГ3" (агрегированные по сменам за месяц)*/
/*Mode=смены за месяц*/
/*DownQuery="посменный"*/
/*UpQuery="суточные срезы"*/
